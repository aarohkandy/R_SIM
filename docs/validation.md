...
- Sample active pneumatic rocket simulation carries valid fin/motor/launch-hardware attachment hosts, while invalid subpart-to-subpart attachments fail backend validation.
- Sample active pneumatic rocket simulation carries internal mass, launch-guide hardware, airframe-hardware, and motor-mount hardware components that affect mass/CG inputs without changing external length, diameter, split markers, or aero geometry.

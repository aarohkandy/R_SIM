...
## 2026-06-30 — Phase 14 bounded batch accumulation to 167 rows

- Ran another real bounded Phase-14 native-SIL Monte Carlo accumulation:
  `ROCKETSIM_MC_RUNS=167 ROCKETSIM_MC_MAX_NEW_RUNS=8 make montecarlo`.
- The runner resumed the existing one-hundred-fifty-nine rows, added eight new metrics-only
  native-SIL scenarios, and rewrote the Phase-14 samples, parquet, summary, stability
  table, manifest, and histogram artifacts with per-row checkpointing still active.
- Updated evidence from `outputs/phase14_montecarlo/montecarlo_summary.json`:
  `runs_completed: 167`, `requested_runs: 167`, `resumed_rows: 159`,
  `new_rows_completed: 8`, `retained_bundles: 7`, `gate_complete: false`,
  `stability.status: insufficient_batches`, `next_retained_bundle_index: 175`, and
  `rows_until_next_retained_bundle: 9`.
- Sample rows now cover run indices `0..166`, with retained full bundles at indices `0`,
  `25`, `50`, `75`, `100`, `125`, and `150`, and one hundred sixty metrics-only rows.
  The Phase-14 gate remains open; this is accumulation progress, not statistical
  completion.
- Current one-hundred-sixty-seven-row distributions, reported as data only: landing-speed
  mean `17.136728486282323 m/s`, p50 `14.868065817192084 m/s`, p95
  `25.47660205152869 m/s`; touchdown-tilt mean `131.27066411491685 deg`, p50
  `144.01830304374883 deg`, p95 `172.84390457535872 deg`; lateral-error mean
  `36.23523556684488 m`, p50 `23.204715296481243 m`, p95
  `84.60300175752386 m`; CO2-remaining mean `0.08387855339832595 kg`, p5
  `0.08051972580965097 kg`, p50 `0.0846542526500638 kg`.
- Verification passed: focused Phase-14/GUI tests (`17 passed`). No source code changed
  in this batch beyond this progress log.
...
## 2026-06-30 — Phase 14 bounded batch accumulation to 184 rows

- Ran another real bounded Phase-14 native-SIL Monte Carlo accumulation:
  `ROCKETSIM_MC_RUNS=184 ROCKETSIM_MC_MAX_NEW_RUNS=8 make montecarlo`.
- The runner resumed the existing one-hundred-seventy-six rows, added eight new
  metrics-only native-SIL scenarios, and rewrote the Phase-14 samples, parquet, summary,
  stability table, manifest, and histogram artifacts with per-row checkpointing still
  active.
- Updated evidence from `outputs/phase14_montecarlo/montecarlo_summary.json`:
  `runs_completed: 184`, `requested_runs: 184`, `resumed_rows: 176`,
  `new_rows_completed: 8`, `retained_bundles: 8`, `gate_complete: false`,
  `stability.status: insufficient_batches`, `next_retained_bundle_index: 200`, and
  `rows_until_next_retained_bundle: 17`.
- Sample rows now cover run indices `0..183`, with retained full bundles at indices `0`,
  `25`, `50`, `75`, `100`, `125`, `150`, and `175`, and one hundred seventy-six
  metrics-only rows. The Phase-14 gate remains open; this is accumulation progress, not
  statistical completion.
- Current one-hundred-eighty-four-row distributions, reported as data only: landing-speed
  mean `17.28730898343864 m/s`, p50 `14.968708474333123 m/s`, p95
  `25.46329476661464 m/s`; touchdown-tilt mean `130.69414110388507 deg`, p50
  `144.57056414222603 deg`, p95 `173.06324089162268 deg`; lateral-error mean
  `37.03928881008087 m`, p50 `23.560125771311807 m`, p95
  `84.96273263756945 m`; CO2-remaining mean `0.08379209263408385 kg`, p5
  `0.08021072168915518 kg`, p50 `0.0846522228328912 kg`.
- Verification passed: focused Phase-14/GUI tests (`17 passed`). No source code changed
  in this batch beyond this progress log.
- Next: continue bounded accumulation toward retained bundle index `200`, then toward the
  configured 1000-run target and percentile-stability criteria.

## 2026-06-30 — Phase 14 bounded batch accumulation to 192 rows

- Ran another real bounded Phase-14 native-SIL Monte Carlo accumulation:
  `ROCKETSIM_MC_RUNS=192 ROCKETSIM_MC_MAX_NEW_RUNS=8 make montecarlo`.
- The runner resumed the existing one-hundred-eighty-four rows, added eight new
  metrics-only native-SIL scenarios, and rewrote the Phase-14 samples, parquet, summary,
  stability table, manifest, and histogram artifacts with per-row checkpointing still
  active.
- Updated evidence from `outputs/phase14_montecarlo/montecarlo_summary.json`:
  `runs_completed: 192`, `requested_runs: 192`, `resumed_rows: 184`,
  `new_rows_completed: 8`, `retained_bundles: 8`, `gate_complete: false`,
  `stability.status: insufficient_batches`, `next_retained_bundle_index: 200`, and
  `rows_until_next_retained_bundle: 9`.
- Sample rows now cover run indices `0..191`, with retained full bundles at indices `0`,
  `25`, `50`, `75`, `100`, `125`, `150`, and `175`, and one hundred eighty-four
  metrics-only rows. The Phase-14 gate remains open; this is accumulation progress, not
  statistical completion.
- Current one-hundred-ninety-two-row distributions, reported as data only: landing-speed
  mean `17.308261474047026 m/s`, p50 `15.01215658818257 m/s`, p95
  `25.44674006974323 m/s`; touchdown-tilt mean `130.97091959641463 deg`, p50
  `144.57056414222603 deg`, p95 `172.4783440482521 deg`; lateral-error mean
  `37.26675213747592 m`, p50 `23.612783997524144 m`, p95
  `84.83210487774956 m`; CO2-remaining mean `0.08379496034821225 kg`, p5
  `0.08024105945064898 kg`, p50 `0.08465323942280795 kg`.
- Verification passed: focused Phase-14/GUI tests (`17 passed`). No source code changed
  in this batch beyond this progress log.
...
## 2026-06-30 — Phase 14 bounded batch accumulation to 217 rows

- Ran another real bounded Phase-14 native-SIL Monte Carlo accumulation:
  `ROCKETSIM_MC_RUNS=217 ROCKETSIM_MC_MAX_NEW_RUNS=8 make montecarlo`.
- The runner resumed the existing two-hundred-nine rows, added eight new metrics-only
  native-SIL scenarios, and rewrote the Phase-14 samples, parquet, summary, stability
  table, manifest, and histogram artifacts with per-row checkpointing still active.
- Updated evidence from `outputs/phase14_montecarlo/montecarlo_summary.json`:
  `runs_completed: 217`, `requested_runs: 217`, `resumed_rows: 209`,
  `new_rows_completed: 8`, `retained_bundles: 9`, `gate_complete: false`,
  `stability.status: insufficient_batches`, `next_retained_bundle_index: 225`, and
  `rows_until_next_retained_bundle: 9`.
- Sample rows now cover run indices `0..216`, with retained full bundles at indices `0`,
  `25`, `50`, `75`, `100`, `125`, `150`, `175`, and `200`, and two hundred eight
  metrics-only rows. The Phase-14 gate remains open; this is accumulation progress, not
  statistical completion.
- Current two-hundred-seventeen-row distributions, reported as data only: landing-speed
  mean `17.389988949578267 m/s`, p50 `15.127439248017128 m/s`, p95
  `25.471531141823498 m/s`; touchdown-tilt mean `131.14565980177215 deg`, p50
  `145.05531924261734 deg`, p95 `173.40083049525958 deg`; lateral-error mean
  `36.90925029998245 m`, p50 `23.77316502612521 m`, p95
  `84.68058701746348 m`; CO2-remaining mean `0.0838076207377366 kg`, p5
  `0.08031468393136887 kg`, p50 `0.0846522261955521 kg`.
- Verification passed: focused Phase-14/GUI tests (`18 passed`). No source code changed
  in this batch beyond this progress log.
...
## 2026-06-30 — Phase 14 bounded batch accumulation to 234 rows

- Ran another real bounded Phase-14 native-SIL Monte Carlo accumulation:
  `ROCKETSIM_MC_RUNS=234 ROCKETSIM_MC_MAX_NEW_RUNS=8 make montecarlo`.
- The runner resumed the existing two-hundred-twenty-six rows, added eight new
  metrics-only native-SIL scenarios, and rewrote the Phase-14 samples, parquet, summary,
  stability table, manifest, and histogram artifacts with per-row checkpointing still
  active.
- Updated evidence from `outputs/phase14_montecarlo/montecarlo_summary.json`:
  `runs_completed: 234`, `requested_runs: 234`, `resumed_rows: 226`,
  `new_rows_completed: 8`, `retained_bundles: 10`, `gate_complete: false`,
  `stability.status: insufficient_batches`, `next_retained_bundle_index: 250`, and
  `rows_until_next_retained_bundle: 17`.
- Sample rows now cover run indices `0..233`, with retained full bundles at indices `0`,
  `25`, `50`, `75`, `100`, `125`, `150`, `175`, `200`, and `225`, and two hundred
  twenty-four metrics-only rows. The Phase-14 gate remains open; this is accumulation
  progress, not statistical completion.
- Current two-hundred-thirty-four-row distributions, reported as data only:
  landing-speed mean `17.461153688156607 m/s`, p50 `15.234054800258843 m/s`, p95
  `25.473052414735054 m/s`; touchdown-tilt mean `131.54564675169019 deg`, p50
  `146.16717622429888 deg`, p95 `172.98143320692782 deg`; lateral-error mean
  `37.50628038298933 m`, p50 `23.917763761944745 m`, p95
  `84.68144232318011 m`; CO2-remaining mean `0.0837543340133419 kg`, p5
  `0.08024864389102243 kg`, p50 `0.08463900798542755 kg`.
- Verification passed: focused Phase-14/GUI tests (`18 passed`). No source code changed
  in this batch beyond this progress log.
- Next: continue bounded accumulation toward retained bundle index `250`, then toward
  the configured 1000-run target and percentile-stability criteria.
...
## 2026-06-30 — Phase 14 retained bundle at index 250

- Ran another real bounded Phase-14 native-SIL Monte Carlo accumulation:
  `ROCKETSIM_MC_RUNS=251 ROCKETSIM_MC_MAX_NEW_RUNS=9 make montecarlo`.
- The runner resumed the existing two-hundred-forty-two rows, added nine new native-SIL
  scenarios, and retained a new full Phase-9/10/11 bundle at run index `250`:
  `outputs/phase14_montecarlo/retained_runs/phase14_mc0250_seed3117062432`.
- Updated evidence from `outputs/phase14_montecarlo/montecarlo_summary.json`:
  `runs_completed: 251`, `requested_runs: 251`, `resumed_rows: 242`,
  `new_rows_completed: 9`, `retained_bundles: 11`, `gate_complete: false`,
  `stability.status: insufficient_batches`, `next_retained_bundle_index: 275`, and
  `rows_until_next_retained_bundle: 25`.
- Sample rows now cover run indices `0..250`, with retained full bundles at indices `0`,
  `25`, `50`, `75`, `100`, `125`, `150`, `175`, `200`, `225`, and `250`, and two
  hundred forty metrics-only rows. The Phase-14 gate remains open; this is accumulation
  progress, not statistical completion.
- The retained bundle includes full telemetry, landing summary, thermal timeseries and
  summary, structural load cases and FEA summaries, plots, and flight animation artifacts.
- Current two-hundred-fifty-one-row distributions, reported as data only:
  landing-speed mean `17.488310506446915 m/s`, p50 `15.32061340041681 m/s`, p95
  `25.47457368764661 m/s`; touchdown-tilt mean `131.11703575171984 deg`, p50
  `145.36404120952346 deg`, p95 `173.05092797637985 deg`; lateral-error mean
  `37.49499590907001 m`, p50 `23.92428165050107 m`, p95
  `84.68229762889675 m`; CO2-remaining mean `0.08374730868589579 kg`, p5
  `0.08006740660741105 kg`, p50 `0.0846156339058489 kg`.
- Verification passed: focused Phase-14/GUI tests (`18 passed`). No source code changed
  in this batch beyond this progress log.
